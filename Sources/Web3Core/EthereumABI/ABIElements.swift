...
extension ABI.Element.Function {
    ...
    public func decodeErrorResponse(_ data: Data, errors: [String: ABI.Element.EthError]? = nil) -> [String: Any]? {
        ...
        if data.count >= 100,
           ...
           let message = String(bytes: Array(data)[68..<(68+messageLength)], encoding: .utf8),
           (68+messageLength == data.count || Array(data)[68+messageLength..<data.count].reduce(0) { $0 + $1 } == 0) {
